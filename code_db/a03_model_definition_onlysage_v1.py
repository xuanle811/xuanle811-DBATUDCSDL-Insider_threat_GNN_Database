"""
mô hình ban đầu chưa cải tiến để tăng inductive test
...
class InsiderThreatDetector(nn.Module):
    ...
    def _encode_snapshot(self, snapshot) -> torch.Tensor:
        """
        Encode 1 snapshot → session embedding [N_sess, H].

        edge_attr đọc trực tiếp từ snapshot — đảm bảo mỗi snapshot
        dùng đúng edge_attr của chính nó, không bị nhầm sang snapshot khác.
        """
        snap_edge_attr_dict = {
    ...
    def forward_temporal(self, snapshot_list: list) -> torch.Tensor:
        ...
        Routing theo gnn_type — nhất quán train lẫn test:
          SAGE     : encode TẤT CẢ T snapshot → padding N_max → [N,T,H]
                     → TSA (src_key_padding_mask) → classifier
                     • Train T=7 | Test T=5 — đều qua TSA đúng cách
                     • Nhãn y chỉ lấy từ snapshot[-1] — T-6..T-1 là context
                     • edge_attr đọc trực tiếp từ từng snapshot trong _encode_snapshot
          GAT/RGCN : encode snapshot[-1] → classifier (không TSA)

        Lưu ý: edge_attr_dict KHÔNG truyền từ ngoài vào.
        _encode_snapshot tự đọc edge_attr từ snapshot.edge_types — đúng với
        từng snapshot, tránh dùng nhầm edge_attr của snapshot khác.
        """
        if self.gnn_type == "sage":
            # encode từng snapshot — edge_attr tự đọc từ snap bên trong
            per_snap = [self._encode_snapshot(s) for s in snapshot_list]

            # Padding N chiều: cần thiết khi test có user/session mới (N_test ≠ N_train)
            ...
            for emb in per_snap:
                ...
                if n < N_max:
                    ...
                    m[n:] = True       # True = padding → bị mask trong TSA
                else:
                    padded.append(emb)
                    m = torch.zeros(N_max, dtype=torch.bool, device=emb.device)
                masks.append(m)

            seq    = torch.stack(padded, dim=1)       # [N_max, T, H]
            tmask  = torch.stack(masks,  dim=1)       # [N_max, T]
            repr_t = self.temporal(seq, mask=tmask)   # [N_max, H]
        else:
            # GAT / RGCN: snapshot[-1], không TSA
