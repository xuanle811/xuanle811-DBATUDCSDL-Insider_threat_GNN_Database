...
def run_phase2(snapshot_list_test: list,
               ...
               tsa_layers: int = 2, dropout: float = 0.3) -> dict:
    """
    Nạp checkpoint phase 1 của cả 3 model, inference trên test graph.
    Không retrain. In bảng so sánh + vẽ figure.

    SAGE     : forward_temporal nhận TẤT CẢ T=5 snapshot → TSA → INDUCTIVE
    GAT/RGCN : forward_temporal nhận snapshot_list_test[-1] → TRANSDUCTIVE
    (routing trong forward_temporal của a03, không cần code thêm ở đây)
    """
    _set_masks_test(snapshot_list_test)
    last_test  = snapshot_list_test[-1]
    test_mask  = last_test['session'].test_mask
    criterion  = FocalLoss(2.0, _pos_weight(last_test['session'].y))
    all_results = {}

    log.info("\n" + "="*60)
    log.info("  PHASE 2 — INDUCTIVE INFERENCE (test graph, 15 ngày, user mới)")
    log.info("="*60)
    ...
    for gnn_type in GNN_TYPES:
        ...
        # Build model với metadata của test graph (user/session mới)
        model = _build_model(gnn_type, last_test, hidden_dim, num_heads, tsa_layers, dropout)
        # strict=False: test graph có thể có thêm node type mới (user mới)
        ...
        t0  = time.perf_counter()
        res = evaluate(model, snapshot_list_test, test_mask,
                       criterion, threshold=best_threshold)
        inf_t = time.perf_counter() - t0

        inductive_tag = ("INDUCTIVE ✓ — generalize sang user/session chưa thấy"
                         if gnn_type == "sage"
                         else "TRANSDUCTIVE ✗ — graph mới cần retrain")
        res.update(gnn_type=gnn_type, inf_time_ms=round(inf_t * 1000, 2))
        all_results[gnn_type] = res

        _print_report(f"PHASE 2 — {gnn_type.upper()}  [{inductive_tag}]",
                      test_mask, res)
        log.info(f"    Inference time: {inf_t*1000:.2f} ms")

    # ── Bảng so sánh phase 2 ─────────────────────────────────────────────
    _print_comparison_table("PHASE 2 — SO SÁNH INDUCTIVE TEST (test graph)", all_results)

    # ── Figure: F1 bar + ROC curve ────────────────────────────────────────
    _plot_results("phase2_inductive_test", all_results,
                  title_bar="F1 — Inductive Test (Test Graph)",
                  title_roc="ROC — Inductive Test (Test Graph)")
