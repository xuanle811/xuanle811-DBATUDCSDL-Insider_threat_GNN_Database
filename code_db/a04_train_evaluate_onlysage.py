...
# Phase 1 với neighbor sampling + calibration
python a04_train_evaluate_onlysage.py --phase 1 --neighbors 10 5

# Phase 2 với adaptive threshold Youden's J
python a04_train_evaluate_onlysage.py --phase 2 --neighbors 10 5 --adaptive_th youden

# Tắt calibration nếu muốn ablation
python a04_train_evaluate_onlysage.py --phase 2 --no_calibration
...
from a03_model_definition_onlysage import InsiderThreatDetector
...
def find_adaptive_threshold(y_true: np.ndarray, y_prob: np.ndarray,
                            strategy: str = "f1") -> tuple:
    """
    Tự động tìm threshold tối ưu TRỰC TIẾP trên tập test/inference.

    Dùng khi phân phối test khác train — tránh áp threshold của train
    lên test dẫn đến precision/recall lệch.

    Chiến lược:
        "f1"      : tối đa hoá F1 (mặc định, giống find_best_threshold).
        "f1_beta" : tối đa hoá F-beta (β=2, ưu tiên recall — phù hợp
                    bài toán phát hiện mối đe doạ nội bộ nơi FN nguy hiểm).
        "balanced": tối đa hoá trung bình hình học Precision * Recall.
        "youden"  : tối đa hoá Youden's J = TPR - FPR (từ ROC curve).

    Returns:
        (best_threshold, best_score, strategy_used)
    """
    if len(np.unique(y_true)) < 2:
        # Chỉ có 1 class → không tính được — fallback về 0.5
        return 0.5, 0.0, strategy
    ...
    if strategy == "youden":
        ...
        best_th  = float(np.clip(best_th, 0.05, 0.95))
        return best_th, float(j_scores[best_idx]), strategy
    ...
    return best_th, best_score, strategy
...
def _build_model(gnn_type: str, snapshot,
                 ...
                 tsa_layers: int, dropout: float,
                 num_neighbors_per_layer: list = None,
                 use_calibration: bool = True) -> nn.Module:
    return InsiderThreatDetector(
        ...
        num_neighbors_per_layer = num_neighbors_per_layer,  # chỉ SAGE dùng
        use_calibration         = use_calibration,          # chỉ SAGE dùng
    )


def _set_masks_test(snapshot_list: list) -> None:
    """Gán train/val=False, test=True cho user trong MỌI snapshot test."""
    for snap in snapshot_list:
        N = snap['user'].x.size(0)
        snap['user'].train_mask = torch.zeros(N, dtype=torch.bool)
        snap['user'].val_mask   = torch.zeros(N, dtype=torch.bool)
        snap['user'].test_mask  = torch.ones(N,  dtype=torch.bool)
...
def evaluate(model: nn.Module,
    ...
    mask          : boolean [N_sess] trên snapshot[-1]['user']
...
def _train_single(model: nn.Module, gnn_type: str,
    ...
    last       = snapshot_list[-1]
    train_mask = last['user'].train_mask
    val_mask   = last['user'].val_mask
    test_mask  = last['user'].test_mask
    labels     = last['user'].y
...
def run_phase1(snapshot_list: list,
               ...
               tsa_layers: int = 2, dropout: float = 0.3,
               num_neighbors_per_layer: list = None,
...
def run_phase2(snapshot_list_test: list,
               ...
               adaptive_threshold_strategy: str = "f1_beta") -> dict:
    """
    ...
    Adaptive Threshold (SAGE inductive):
        Thay vì áp threshold từ train (bị lệch do phân phối test khác),
        tự tìm threshold tối ưu trên chính y_prob của tập test.
        strategy = "f1_beta" → ưu tiên recall (β=2), phù hợp insider threat.
        Các model transductive vẫn dùng best_threshold từ checkpoint.
    ...
    log.info(f"  SAGE adaptive threshold strategy: '{adaptive_threshold_strategy}'")
    ...
    for gnn_type in GNN_TYPES:
        ...
        log.info(f"\n  [{gnn_type.upper()}] epoch={ckpt['epoch']} | "
                 f"val_f1={ckpt['val_f1']:.4f} | train_threshold={best_threshold:.3f}")

        # ── Dùng pos_weight của snapshot cuối để tính loss ──────────────
        criterion = FocalLoss(2.0, _pos_weight(last_test['user'].y))

        # ── Sliding-window inference ─────────────────────────────────────
        # Bước 1: Collect tất cả y_prob/y_true từ window cuối (đủ context nhất)
        # dành riêng cho SAGE adaptive threshold
        window_results = []
        ...
        if gnn_type == "sage":
            ...
            # Bước 1: thu thập y_prob/y_true với threshold tạm = 0.5 (để tính adaptive sau)
            for end in range(1, M + 1):
                window = snapshot_list_test[:end]
                res_w  = _infer_one_snapshot(model, gnn_type, window,
                                             threshold=0.5,   # placeholder, thay sau
                                             criterion=criterion)
                window_results.append(res_w)

            # ── Adaptive Threshold: tìm trên y_prob của window cuối ──────
            final_y_true = window_results[-1]['y_true']
            final_y_prob = window_results[-1]['y_prob']
            adapt_th, adapt_score, _ = find_adaptive_threshold(
                final_y_true, final_y_prob, strategy=adaptive_threshold_strategy)

            log.info(f"    [SAGE] Adaptive threshold ({adaptive_threshold_strategy}): "
                     f"{adapt_th:.3f} (score={adapt_score:.4f}) "
                     f"← thay thế train threshold ({best_threshold:.3f})")

            # Bước 2: tái tính metrics cho TẤT CẢ window bằng adaptive threshold
            for res_w in window_results:
                y_t = res_w['y_true']
                y_p = res_w['y_prob']
                y_pred = (y_p >= adapt_th).astype(int)
                res_w['threshold']  = adapt_th
                res_w['accuracy']   = accuracy_score(y_t, y_pred)
                res_w['precision']  = precision_score(y_t, y_pred, zero_division=0)
                res_w['recall']     = recall_score(y_t, y_pred, zero_division=0)
                res_w['f1']         = f1_score(y_t, y_pred, zero_division=0)

            for end, res_w in enumerate(window_results, start=1):
                log.info(f"    Window [:{end:2d}] | "
                         f"F1={res_w['f1']:.4f} | "
                         f"AUC={res_w['auc_roc']:.4f} | "
                         f"Rec={res_w['recall']:.4f} | Th={res_w['threshold']:.3f}")
        else:
            # Single snapshot: [0], [1], ..., [M-1]
            # GAT/RGCN forward_temporal tự lấy snapshot[-1] → mỗi lần encode 1 snap
            # Giữ nguyên threshold từ train (không đổi logic transductive)
            for idx in range(M):
...
if __name__ == "__main__":
    ...
    # ── Cải tiến inductive ─────────────────────────────────────────────────
    parser.add_argument("--neighbors",  type=int,   nargs=2,   default=None,
                        metavar=("K1", "K2"),
                        help="Neighbor sampling per layer, vd: --neighbors 10 5. "
                             "None = không sample (dùng toàn bộ neighbor).")
    parser.add_argument("--no_calibration", action="store_true", default=False,
                        help="Tắt Calibration Embedding (mặc định: bật).")
    parser.add_argument("--adaptive_th", type=str, default="f1_beta",
                        choices=["f1", "f1_beta", "balanced", "youden"],
                        help="Chiến lược adaptive threshold cho SAGE phase 2 "
                             "(mặc định: f1_beta — ưu tiên recall).")
    args = parser.parse_args()

    use_calibration = not args.no_calibration
    log.info(f"  Settings: neighbors={args.neighbors} | "
             f"calibration={use_calibration} | adaptive_th={args.adaptive_th}")

    # ── Load train graph ──────────────────────────────────────────────────
    try:
        snapshot_list = torch.load(HETERO_GRAPH_PATH, weights_only=False)
        N_last = snapshot_list[-1]['user'].x.size(0)
        log.info(f"Train graph: '{HETERO_GRAPH_PATH}' | "
                 f"{len(snapshot_list)} snaps | {N_last} users (last)")
    ...
    try:
        ...
        _set_masks_test(snapshot_list_test)
        N_test = snapshot_list_test[-1]['user'].x.size(0)
        log.info(f"Test graph : '{HETERO_TEST_GRAPH_PATH}' | "
                 f"{len(snapshot_list_test)} snaps | {N_test} users (last)")
    except FileNotFoundError:
    ...
    if args.phase == 1:
        run_phase1(snapshot_list,
                   ...
                   num_neighbors_per_layer  = args.neighbors,
                   use_calibration          = use_calibration)
        ...
        run_phase2(snapshot_list_test,
                   ...
                   num_neighbors_per_layer       = args.neighbors,
                   use_calibration               = use_calibration,
                   adaptive_threshold_strategy   = args.adaptive_th)
